...
test('node path resolution', () => {
  ...
  expect(n1.type).toEqual(types.OBJECT);

  expect(n1.path([]).value).toEqual(o1);
  expect(n1.path([]).type).toEqual(types.OBJECT);

  expect(n1.path(['a', 'b', 'c']).value).toEqual(123);
  expect(n1.path(['a', 'b', 'c']).type).toEqual(types.NUMBER);
  ...
  expect(n2.type).toEqual(types.ARRAY);
  expect(n2.path([]).value).toEqual([{ a: 1 }]);
  expect(n2.path([]).type).toEqual(types.ARRAY);

  expect(n2.path([0]).value).toEqual({ a: 1 });
  expect(n2.path([0]).type).toEqual(types.OBJECT);

  expect(n2.path([0, 'a']).value).toEqual(1);
  expect(n2.path([0, 'a']).type).toEqual(types.NUMBER);
  ...
  expect(n3.type).toEqual(types.ARRAY);

  expect(n3.path([]).value).toEqual(o3);
  expect(n3.path([]).type).toEqual(types.ARRAY);

  expect(n3.path([0, 'a', 2]).value).toEqual( { b: [3, 4, 5] });
  expect(n3.path([0, 'a', 2]).type).toEqual(types.OBJECT);

  expect(n3.path([0, 'a', 2, 'b', 2]).value).toEqual(5);
  expect(n3.path([0, 'a', 2, 'b', 2]).type).toEqual(types.NUMBER);
...
test('comparisons', () => {
  const compare = (v1, v2) => new Node(v1).compare(v2);
